...
const Navbar = () => {
  ...
  const handleNavClick = (link: typeof navLinks[0]) => {
    ...
    // Hash link: if on homepage, scroll; otherwise navigate to /#hash
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-5 py-3.5">
        ...
        <div className="hidden md:flex items-center gap-7">
          {navLinks.map((l) => (
            <button key={l.label} onClick={() => handleNavClick(l)} className="text-sm text-muted-foreground hover:text-foreground transition-colors duration-300 bg-transparent border-none cursor-pointer">
              {l.label}
            </button>
          ))}
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="md:hidden overflow-hidden bg-background/95 backdrop-blur-2xl border-t border-border/30">
            <div className="flex flex-col gap-4 px-5 py-6">
              {navLinks.map((l) => (
                <button key={l.label} onClick={() => handleNavClick(l)} className="text-left text-muted-foreground hover:text-foreground transition-colors bg-transparent border-none cursor-pointer">
                  {l.label}
                </button>
              ))}
